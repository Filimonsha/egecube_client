        ...
        <div>
            ...
            >
                {["текст", "один", "несколько", "документ"].map((title, index) => {
                    return <ToggleButton key={index} value={Object.keys(ETaskType)[index]}>{title}</ToggleButton>
                })}
            </ToggleButtonGroup>

            {
                taskType === ETaskType.TEXT_INPUT ?
                    <TextField/> :
                    taskType === ETaskType.SINGLE_CHOICE || ETaskType.MULTIPLE_ANSWER &&
                    <AnswerVariants variants={taskVariants} setVariants={setTaskVariants} answerType={taskType}/>
            }
        </div>
...
const HomeworkCreator = ({courses}: IHomeworkCreatorProps) => {
    ...
    return (
        <LocalizationProvider dateAdapter={AdapterDayjs}>
            ...
            <Box sx={{flexGrow: 1}}>
                <Grid container={true} rowSpacing={2} columnSpacing={{xs: 1, sm: 2, md: 3}}>
                    <Grid2 xs={6}>
                        <div>
                            <TextField fullWidth value={title} onChange={value => setTitle(value.target.value)}/>
            ...
            <div>
                ...
                >
                    <Box sx={style}>
                        ...
                        <Button onClick={() => setCreateNewTasks(prevState => !prevState)}>Создать задание</Button>
                    </Box>
                </Modal>
            </div>
